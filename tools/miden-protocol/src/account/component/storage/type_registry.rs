...
mod tests {
    ...
    fn auth_scheme_type_supports_named_and_numeric_values() {
        ...
        assert_eq!(numeric_word, Word::from([Felt::new(2u8).expect("invalid field element"), Felt::ZERO, Felt::ZERO, Felt::ZERO]));

        let named_word = SCHEMA_TYPE_REGISTRY
            .try_parse_word(&auth_scheme_type, "EcdsaK256Keccak")
            .expect("named auth scheme should parse");
        assert_eq!(named_word, Word::from([Felt::new(1u8).expect("invalid field element"), Felt::ZERO, Felt::ZERO, Felt::ZERO]));
    ...
    fn schema_types_reject_invalid_values() {
        ...
        let invalid_word = Word::from([Felt::new(9u8).expect("invalid field element"), Felt::ZERO, Felt::ZERO, Felt::ZERO]);
        assert!(
        ...
        assert_eq!(SCHEMA_TYPE_REGISTRY.try_parse_felt(&bool_type, "true").unwrap(), Felt::new(1).expect("invalid field element"));
        assert_eq!(SCHEMA_TYPE_REGISTRY.try_parse_felt(&bool_type, "false").unwrap(), Felt::new(0).expect("invalid field element"));
        assert_eq!(SCHEMA_TYPE_REGISTRY.try_parse_felt(&bool_type, "1").unwrap(), Felt::new(1).expect("invalid field element"));
        assert_eq!(SCHEMA_TYPE_REGISTRY.try_parse_felt(&bool_type, "0").unwrap(), Felt::new(0).expect("invalid field element"));
        assert_eq!(SCHEMA_TYPE_REGISTRY.display_felt(&bool_type, Felt::new(0).expect("invalid field element")), "false");
        assert_eq!(SCHEMA_TYPE_REGISTRY.display_felt(&bool_type, Felt::new(1).expect("invalid field element")), "true");
        ...
        assert!(SCHEMA_TYPE_REGISTRY.validate_felt_value(&bool_type, Felt::new(2).expect("invalid field element")).is_err());
    }
}

...
mod tests {
    ...
    fn test_serde_account() {
        let init_nonce = Felt::new(1).expect("invalid field element");
    ...
    fn test_serde_account_delta() {
        ...
        let nonce_delta = Felt::new(2).expect("invalid field element");
    ...
    fn valid_account_delta_is_correctly_applied() {
        ...
        let mut storage_map = StorageMap::with_entries([
            (
                ...
                Word::from([
                    Felt::new(1_u64).expect("invalid field element"),
                    Felt::new(2_u64).expect("invalid field element"),
                    Felt::new(3_u64).expect("invalid field element"),
                    Felt::new(4_u64).expect("invalid field element"),
                ]),
            ),
            (
        ...
        // build account delta
        let final_nonce = Felt::new(2).expect("invalid field element");
        let storage_delta = AccountStorageDelta::new()
            .add_cleared_items([StorageSlotName::mock(0)])
            .add_updated_values([(StorageSlotName::mock(1), Word::from([1, 2, 3, 4u32]))])
    ...
    fn valid_account_delta_with_decremented_nonce() {
        // build account
        let account_id = AccountId::try_from(ACCOUNT_ID_PRIVATE_SENDER).unwrap();
        let init_nonce = Felt::new(2).expect("invalid field element");
        let asset = FungibleAsset::mock(100);
        let mut account =
            build_account(vec![asset], init_nonce, vec![StorageSlotContent::Value(Word::empty())]);

        // build account delta
        let final_nonce = Felt::new(1).expect("invalid field element");
        let storage_delta = AccountStorageDelta::new()
            .add_cleared_items([StorageSlotName::mock(0)])
            .add_updated_values([(StorageSlotName::mock(1), Word::from([1, 2, 3, 4u32]))]);
        let account_delta =
    ...
    fn empty_account_delta_with_incremented_nonce() {
        ...
        // build account delta
        let nonce_delta = Felt::new(1).expect("invalid field element");

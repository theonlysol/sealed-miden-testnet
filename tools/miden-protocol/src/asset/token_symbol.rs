...
impl TokenSymbol {
    ...
    pub fn as_element(&self) -> Felt {
        ...
        Felt::new(encoded_value).expect("invalid field element")
    }
}
...
mod test {
    ...
    fn test_invalid_token_len() {
        ...
        let invalid_encoded_symbol_u64 = Felt::new(encoded_symbol).expect("invalid field element").as_canonical_u64() - 3;

        // check that decoding returns an error for a token with invalid length
        let err = TokenSymbol::try_from(Felt::new(invalid_encoded_symbol_u64).expect("invalid field element")).unwrap_err();
    ...
    fn test_token_symbol_max_value() {
        ...
        assert_eq!(Felt::new(token_symbol).expect("invalid field element").as_canonical_u64(), TokenSymbol::MAX_ENCODED_VALUE);
    }
    ...
    fn test_token_symbol_min_value() {
        ...
        assert_eq!(Felt::new(token_symbol).expect("invalid field element").as_canonical_u64(), TokenSymbol::MIN_ENCODED_VALUE);
    }

...
impl TransactionAdviceInputs {
    ...
    fn build_stack(&mut self, tx_inputs: &TransactionInputs) {
        ...
        self.extend_stack([
            ...
            Felt::new(header.version() as u64).expect("invalid field element"),
            Felt::new(header.timestamp() as u64).expect("invalid field element"),
            ZERO,
        ]);
        self.extend_stack([
            ZERO,
            Felt::new(header.fee_parameters().verification_base_fee() as u64).expect("invalid field element"),
        ...
        self.extend_stack([Felt::new(tx_inputs.input_notes().num_notes() as u64).expect("invalid field element")]);
    ...
    fn add_partial_blockchain(&mut self, mmr: &PartialBlockchain) {
        ...
        let peaks = mmr.peaks();
        let mut elements = vec![Felt::new(peaks.num_leaves() as u64).expect("invalid field element"), ZERO, ZERO, ZERO];
    ...
    fn add_input_notes(&mut self, tx_inputs: &TransactionInputs) {
        ...
        for input_note in tx_inputs.input_notes().iter() {
            ...
            note_data.push(Felt::new(recipient.storage().num_items() as u64).expect("invalid field element"));
            note_data.push(Felt::new(assets.num_assets() as u64).expect("invalid field element"));
            ...
            match input_note {
                InputNote::Authenticated { note, proof } => {
                    ...
                    note_data.push(Felt::new(proof.location().block_note_tree_index() as u64).expect("invalid field element"));
                },

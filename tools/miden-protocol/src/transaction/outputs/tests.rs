...
fn output_note_size_hint_matches_serialized_length() -> anyhow::Result<()> {
    ...
    let storage = NoteStorage::new(vec![Felt::new(1).expect("invalid field element"), Felt::new(2).expect("invalid field element")])?;
...
fn oversized_public_note_triggers_size_limit_error() -> anyhow::Result<()> {
    ...
    for i in 0..7_000_u16 {
        let digest = Word::new([Felt::new(i + 1).expect("invalid field element"), Felt::ZERO, Felt::ZERO, Felt::ZERO]);
